    ...
    <article className="group relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
        ...
        <div className="absolute bottom-3 right-3 z-30 pointer-events-auto sm:bottom-4 sm:right-4">
          <AddToCartButton
            ...
            className={`group border border-white/70 bg-white/95 p-0 text-slate-950 shadow-lg shadow-slate-950/15 hover:-translate-y-0.5 hover:bg-white/95 focus-visible:bg-white/95 active:scale-95 ${
      ...
      <div className="pointer-events-none relative z-20 space-y-2 p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <p className="min-w-0 break-words font-display text-sm font-semibold leading-5 text-slate-950 sm:text-base md:text-lg">
            {product.name}
          </p>
          <span className="shrink-0 text-xs font-semibold text-slate-900 sm:text-sm">{formatCurrency(product.price)}</span>
        </div>
        <div className="space-y-1.5">
          {product.minimumOrderQuantity > 1 ? (
            <p className="text-[0.6rem] font-semibold uppercase tracking-[0.16em] text-slate-500 sm:text-[0.65rem]">
